...
interface ChallengeItemProps {
  ...
  activeColorClass: string   // e.g. 'bg-yellow-500 border-yellow-500'
}
    ...
    <div className="flex items-center gap-3 bg-white/5 p-2 rounded-lg">
      <div
        className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 ${
          completed ? activeColorClass : 'border-gray-600 bg-transparent'
        }`}
      >
        {completed && <CheckCircle className="w-4 h-4 text-black" />}
      </div>
      <span className={`text-md text-left ${completed ? 'text-white' : 'text-gray-500'}`}>
        {label}
      </span>
    </div>
  )
      ...
      <div className="w-full bg-black/40 rounded-xl border border-white/10 p-4 backdrop-blur-sm">
        ...
        <div className="flex flex-col gap-3">
          ...
          <ChallengeItem
            ...
            label={<><span className="font-bold text-blue-400">Quiz Hunter:</span> All questions correct</>}
          />
        </div>
      </div>
